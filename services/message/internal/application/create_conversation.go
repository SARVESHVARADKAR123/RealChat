...
func (s *Service) CreateConversation(
	...
	// 1. Initial best-effort lookup before transaction
	if lookupKey != nil {
	...
	txErr := s.tx.WithTx(ctx, func(ctx context.Context, tx *sql.Tx) error {
		...
		if err != nil {
			// 4. Handle race condition violation on unique lookup_key
			if lookupKey != nil {
...
func (s *Service) emitConversationCreated(ctx context.Context, tx *sql.Tx, convID string) (*domain.Conversation, error) {
	...
	pbConv := &messagingv1.Conversation{
	...
	event := &messagingv1.ConversationCreatedEvent{
		Conversation:       pbConv,
	...
	env := &messagingv1.MessagingEventEnvelope{
		EventType:     messagingv1.MessagingEventType_MESSAGING_EVENT_TYPE_CONVERSATION_CREATED,
		SchemaVersion: 1,
