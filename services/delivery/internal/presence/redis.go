...
func (p *Presence) Register(ctx context.Context, userID, deviceID string) error {
	pipe := p.client.TxPipeline()

	pipe.Set(ctx, sessionKey(userID, deviceID), p.instanceID, TTL)
	pipe.SAdd(ctx, userSessionsSetKey(userID), deviceID)
	pipe.Expire(ctx, userSessionsSetKey(userID), TTL+time.Hour) // Set longer TTL for the set
...
func (p *Presence) Unregister(ctx context.Context, userID, deviceID string) error {
	...
	// Double check if any other devices are online before marking offline
...
func (p *Presence) GetUserDevices(ctx context.Context, userID string) (map[string]string, error) {
	...
	if len(staleDevices) > 0 {
		go func() {
			...
			if err != nil {
				log.Printf("presence: fail to cleanup stale devices for user=%s: %v", userID, err)
			}
		}()
	}
